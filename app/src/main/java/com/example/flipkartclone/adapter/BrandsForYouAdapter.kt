...
class BrandsForYouAdapter(
    private val onClick:(position:Int,itemData: ItemModelItem) -> Unit
):RecyclerView.Adapter<BrandsForYouAdapter.BrandsViewHolder>() {

    inner class BrandsViewHolder(itemView: View):RecyclerView.ViewHolder(itemView) {
        val itemImage:ImageView = itemView.findViewById(R.id.ivBrandsForYou)
        val itemTitle:TextView = itemView.findViewById(R.id.tvBrandsLoveTitle)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BrandsViewHolder {
        return BrandsViewHolder(LayoutInflater.from(parent.context).inflate(
            R.layout.brands_love_item,parent,false))
    }
